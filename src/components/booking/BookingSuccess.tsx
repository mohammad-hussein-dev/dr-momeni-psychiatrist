 ...
 *              seamless Persian (Shamsi) and English (Gregorian) date support.
 *
 * @features
 * - Dynamic Shamsi/Gregorian date formatting based on active language
 * - Client-side generation of .ics-compatible Calendar URLs
 * - Pre-filled WhatsApp message with booking details
 * - Clipboard API integration for tracking ID
 * - Full accessibility compliance (ARIA roles, focus management)
 *
 ...
 * @description Generates a pre-filled Google Calendar event URL
 */
 ...
 * @description Generates a pre-filled Outlook Calendar event URL
 */
 ...
 * @description Generates a pre-filled WhatsApp message URL
 */
...
export const BookingSuccess: React.FC<IBookingSuccessProps> = ({
     ...
     * @description Formats the date to Shamsi (fa-IR) or Gregorian (en-US) based on active language
     */
    ...
    }, [bookingData, bookingResponse.bookingId, isEn]);

    const handleAddToOutlookCalendar = useCallback(() => {
    ...
    return (
        ...
        <span className="font-mono font-bold text-primary bg-primary/10 px-2.5 py-0.5 rounded-md" dir="ltr">
        {bookingResponse.bookingId}
        </span>
        </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 mb-6 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
        ...
        {/* Professional Date Formatting Applied Here */}
        ...
        className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-background border border-border text-sm font-medium text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all duration-300 active:scale-95"
        >
        ...
        className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-success text-success-foreground font-semibold shadow-md hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all duration-300"
        >
        ...
        {bookingResponse.bookingId && (
            ...
            className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-background border border-border text-sm font-medium text-foreground hover:bg-muted transition-all duration-300 active:scale-95"
            >
            {copiedId ? (
                <>
        ...
        {/* ─── Helper Advice Note ───────────────────────────────────────────── */}
        <div className="mt-6 p-4 rounded-2xl bg-primary/5 border border-primary/20 animate-in fade-in duration-1000 delay-300">
