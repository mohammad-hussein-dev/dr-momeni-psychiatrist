/**
 * @file HospitalLocationMap component displaying the clinic's location
 * via a clean, responsive Google Maps iframe, replacing legacy WebGL maps.
 * Includes direct 1-click routing buttons for Iranian navigation apps.
 *
 * @author Mohammad Hossein
 * @version 2.0.0 (Refactored for Light-Mode-Only, Performance, and Accessibility)
 */
import React, { useState } from 'react';
import {
  MapPin, Navigation, ExternalLink, Copy, Check, Car, Phone,
  Building2, ShieldCheck, Compass, Sparkles, Award, Bus, Train,
  Share2, Info, CheckCircle2, CornerDownLeft, Route
} from 'lucide-react';
import { useLanguage } from '../../i18n/LanguageProvider';
import {
  HOSPITAL_NAME_FA, HOSPITAL_NAME_EN, ADDRESS_FA, ADDRESS_EN,
  ADDRESS_EXACT_HINT_FA, ADDRESS_EXACT_HINT_EN, LATITUDE, LONGITUDE,
  NESHAN_SHORT_URL, NESHAN_URL, NESHAN_MAP_URL, GOOGLE_MAPS_URL,
  GOOGLE_MAPS_DIR_URL, WAZE_URL, BALAD_URL, APPLE_MAPS_URL,
  OSM_EMBED_URL, HOSPITAL_URL, DOCTOR_NIKAN_URL, MEDICAL_COUNCIL_FA,
  MEDICAL_COUNCIL_CODE, HOSPITAL_CENTRAL_PHONE_FA, HOSPITAL_CENTRAL_PHONE,
  PHONE, PHONE_TEL
} from '../../lib/siteConstants';

export interface IHospitalLocationMapProps {
  showTitle?: boolean;
  className?: string;
}

/**
 * Accessible, responsive map component with multi-app navigation and route guidance.
 * Enforces light mode styling and strict RTL logical properties.
 *
 * @param props - IHospitalLocationMapProps
 * @returns React.JSX.Element
 */
export const HospitalLocationMap: React.FC<IHospitalLocationMapProps> = ({
  showTitle = true,
  className = ''
}) => {
  ...
  const [copiedNeshanLink, setCopiedNeshanLink] = useState<boolean>(false);
  ...
  const handleCopyCoordinates = (): void => {
    navigator.clipboard.writeText(`${LATITUDE}, ${LONGITUDE}`).catch(console.error);
    setCopiedCoord(true);
    setTimeout(() => setCopiedCoord(false), 2500);
  };

  const handleCopyNeshan = (): void => {
    navigator.clipboard.writeText(NESHAN_SHORT_URL).catch(console.error);
  ...
  const navApps = [
    ...
    },
    {
      ...
      enName: 'Balad',
      url: BALAD_URL,
      iconBg: 'bg-emerald-600/10 text-emerald-700 border-emerald-500/30 hover:bg-emerald-600 hover:text-white',
      badge: 'Iran',
      isPrimary: false
    },
    {
      ...
      enName: 'Google Maps',
      url: GOOGLE_MAPS_URL,
      iconBg: 'bg-blue-500/10 text-blue-600 border-blue-500/30 hover:bg-blue-600 hover:text-white',
      badge: 'Global',
      isPrimary: false
    },
    {
      ...
      enName: 'Waze',
      url: WAZE_URL,
      iconBg: 'bg-cyan-500/10 text-cyan-700 border-cyan-500/30 hover:bg-cyan-600 hover:text-white',
      badge: 'Traffic',
      isPrimary: false
    },
    {
      ...
      enName: 'Apple Maps',
      url: APPLE_MAPS_URL,
      iconBg: 'bg-neutral-500/10 text-neutral-800 border-neutral-500/30 hover:bg-neutral-800 hover:text-white',
      badge: 'iOS',
      isPrimary: false
    }
  ];
  ...
  return (
    ...
    {/* Interactive Map Visual Layer (Clean Google Maps Iframe) */}
    ...
    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3239.5!2d51.27039!3d35.75711!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zNTHCsDE2JzEzLjQiRSAzNcKwNDUnMjUuNiJO!5e0!3m2!1sfa!2sir!4v1234567890"
    ...
    <span>{t('nav_apps_title') || 'برنامه‌های مسیریابی'}</span>
    </h4>
    <div className="flex items-center gap-2">
    <button
    type="button"
    onClick={handleCopyNeshan}
    ...
    {/* ========================================================================= */}
    {/* ROUTE ASSISTANT & DIRECTIONS GUIDE */}
    {/* ========================================================================= */}
    ...
    target="_blank"
    rel="noreferrer"
    className="inline-flex items-center gap-1 text-blue-600 font-bold hover:underline font-mono"
    dir="ltr"
    >
    ...
    {copied ? (
      ...
      <span className="text-emerald-600">{t('nav_copied') || 'کپی شد'}</span>
      </>
    ) : (
      <>
      <Copy className="w-3 h-3" />
      <span>{t('nav_copy_address') || 'کپی آدرس'}</span>
      </>
    )}
    </button>
