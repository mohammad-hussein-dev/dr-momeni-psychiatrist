...
export const HospitalLocationMap: React.FC<HospitalLocationMapProps> = ({
  ...
  const navApps = [
    {
      name: lang === 'fa' ? 'مسیریابی نشان (تایید شده)' : 'Neshan (Verified)',
      enName: 'Neshan',
      url: NESHAN_URL,
      iconBg: 'bg-blue-600/15 text-blue-700 dark:text-blue-300 border-blue-600/40 hover:bg-blue-600 hover:text-white ring-1 ring-blue-500/30 font-bold',
      badge: 'Neshan Direct'
    },
    {
    ...
    },
    {
      ...
      enName: 'Google Maps',
      url: GOOGLE_MAPS_URL,
      iconBg: 'bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/20 hover:bg-sky-600 hover:text-white',
      badge: 'Global'
    },
    {
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="rounded-2xl sm:rounded-3xl bg-card border border-border/80 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-5 md:p-6 bg-card space-y-4 sm:space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2.5 mt-1.5 text-[11px] text-muted-foreground">
                  ...
                  <button
                    ...
                    title={lang === 'fa' ? 'کپی مختصات جغرافیایی دقیق' : 'Copy GPS coordinates'}
                  >
          ...
          {/* DEDICATED PARKING GUIDANCE & DIRECT ENTRANCE CARD */}
          <div className="p-4 sm:p-4.5 rounded-2xl bg-secondary/15 border border-secondary/35 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h5 className="font-heading font-bold text-foreground text-xs sm:text-sm flex items-center gap-1.5">
                    <span>{lang === 'fa' ? PARKING_NAME_FA : PARKING_NAME_EN}</span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary text-primary-foreground">
                      {lang === 'fa' ? 'مستقر در محل بیمارستان' : 'On-Site'}
                    </span>
                  </h5>
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    {lang === 'fa' ? PARKING_ENTRANCE_FA : PARKING_ENTRANCE_EN}
                  </p>
                </div>
              </div>

              {/* Direct Parking Quick-Nav */}
              <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                <a
                  href={NESHAN_URL}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-primary text-primary-foreground text-[11px] font-bold hover:opacity-90 shadow-2xs transition-all"
                >
                  <Navigation className="w-3 h-3" />
                  <span>{lang === 'fa' ? 'مسیریابی پارکینگ در نشان' : 'Neshan'}</span>
                </a>
                <a
                  href={BALAD_URL}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-card border border-border text-foreground text-[11px] font-semibold hover:border-primary hover:text-primary transition-all"
                >
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-card border border-border text-foreground text-[11px] font-semibold hover:border-primary hover:text-primary transition-all"
                >
                  <span>{lang === 'fa' ? 'گوگل مپ' : 'Google'}</span>
                </a>
              </div>
            </div>

            {/* Parking Features Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-2 border-t border-secondary/25">
              {(lang === 'fa' ? PARKING_FEATURES_FA : PARKING_FEATURES_EN).map((feature, idx) => (
