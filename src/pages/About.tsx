 ...
 * @description Displays Dr. Fatemeh Momeni's professional biography, treatment philosophy,
 *              and clinical credentials. Utilizes the centralized asset registry for
 *              resilient, accessible, and SEO-optimized image loading.
 *
 * @features
 * - Centralized asset management via `assetRegistry.ts`
 * - Bilingual (FA/EN) dynamic alt-text and content rendering
 * - Responsive grid layouts with scroll-reveal animations
 * - Accessibility-compliant image tags with explicit dimensions (CLS prevention)
 *
 * @author Mohammad Hossein (Senior Frontend Engineer)
 * @version 3.0.0 (Asset Registry Integration)
 * @since 2026-09-10
 */
...
export const About: React.FC = () => {
  ...
  return (
    ...
    <AppImage
    src={DOCTOR_ASSETS.about.src}
    alt={lang === 'fa' ? DOCTOR_ASSETS.about.altFa : DOCTOR_ASSETS.about.altEn}
    width={DOCTOR_ASSETS.about.width}
    height={DOCTOR_ASSETS.about.height}
    ...
    {/* Top Verified Badge */}
    <div className="absolute top-3 sm:top-4 start-2 sm:start-3 glass-card rounded-full px-3.5 py-1.5 shadow-md border border-primary/25 bg-card/95 backdrop-blur-md flex items-center gap-1.5 text-xs font-semibold text-primary z-10">
    <Award className="w-4 h-4 text-primary" />
    <span>{t('board_certified')}</span>
    </div>

    {/* Bottom Floating Hospital Card */}
    <div className="absolute -bottom-3 sm:-bottom-4 end-2 sm:end-3 glass-card rounded-2xl px-3.5 py-2.5 sm:px-4 sm:py-3 shadow-xl border border-border/80 bg-card/95 backdrop-blur-md flex items-center gap-3 z-10 max-w-[calc(100%-1rem)]">
    ...
    <AppImage
    src={PHILOSOPHY_ASSETS.clinicalFlowPathway.src}
    alt={lang === 'fa' ? PHILOSOPHY_ASSETS.clinicalFlowPathway.altFa : PHILOSOPHY_ASSETS.clinicalFlowPathway.altEn}
    width={PHILOSOPHY_ASSETS.clinicalFlowPathway.width}
    height={PHILOSOPHY_ASSETS.clinicalFlowPathway.height}
